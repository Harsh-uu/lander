...
// app/component/SVGParticles.tsx
...
function getTransitionParams(tr) {
  ...
  if (tr.type === "spring") {
    const k = (_a = tr.stiffness) != null ? _a : 100, d = (_b = tr.damping) != null ? _b : 15, m = (_c = tr.mass) != null ? _c : 1;
    const durMs = Math.min(
      3e3,
      Math.max(300, d / (2 * Math.sqrt(k * m)) * 2e3)
    );
    return { easeFn: EASE.backOut, durMs };
  }
  return {
    easeFn: EASE[tr.ease] || EASE.easeOut,
    durMs: ((_d = tr.duration) != null ? _d : 0.8) * 1e3
  };
}
...
function ParticleImage(_a) {
    ...
    const tryLoad = (cors) => {
      ...
      img.onload = () => {
        ...
        if (!hOn) {
          ...
          particles = src.map(
            (p) => mkParticle(p, p.homeX, p.homeY, p.homeX, p.homeY)
          );
        } else if (ht === "roam") {
          const bw = Math.max(80, rw || W), bh = Math.max(80, rh || H);
          const bx = (W - bw) / 2, by = (H - bh) / 2;
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (active) {
        const lerpFactor = Math.max(0.08, 0.3 - hitSpeed * 6e-3);
        if (sm.x < -9e3) {
          sm.x = rawMx;
          sm.y = rawMy;
        } else {
      ...
      for (const p of particles) {
        ...
        if (state === "assembling") {
          ...
          if (ht === "roam") {
            ...
            p.vx = (p.vx || 0) * 0.98 + (p.roamTargetX - p.x) * 3e-3;
            p.vy = (p.vy || 0) * 0.98 + (p.roamTargetY - p.y) * 3e-3;
      ...
      children: [
        ...
        !image && /* @__PURE__ */ jsx(
          "div",
          {
            ...
            children: "Upload image in panel \u2192"
          }
        )
      ]
    })
...
addPropertyControls(ParticleImage, {
  imageConfig: {
    ...
    controls: {
      ...
      widthPx: {
        ...
        step: 1,
        unit: "px",
        hidden: (props) => props.mode === "fit" || props.sizeUnit !== "px"
      },
      heightPx: {
        type: ControlType.Number,
        title: "Height",
  ...
  particleShape: {
    ...
    options: ["square", "circle", "both"],
    optionTitles: ["Square", "Circle", "Both"],
    defaultValue: "square",
  ...
  particleColor: {
    ...
    options: ["original", "single", "multi"],
    optionTitles: ["Original", "Single", "Multi"],
  ...
  singleColor: {
    ...
    title: "Color",
    defaultValue: "#ffffff",
    hidden: (props) => props.particleColor !== "single"
  },
  multiColors: {
    ...
    maxCount: 10,
    defaultValue: ["#ffffff", "#aaaaaa", "#555555"],
    hidden: (props) => props.particleColor !== "multi"
  },
  particleCount: {
    type: ControlType.Number,
  ...
  hoverConfig: {
    ...
    controls: {
      hoverType: {
        ...
        title: "Type",
        options: ["roam", "hide"],
        optionTitles: ["Roam", "Hide"]
      },
      transition: {
        type: ControlType.Transition,
        title: "Transition",
        defaultValue: { duration: 0.8, ease: "easeOut" }
      },
      roamWidth: {
        type: ControlType.Number,
        title: "Roam Width",
      ...
      roamHeight: {
        ...
        max: 4e3,
        step: 10,
        hidden: (p) => p.hoverType !== "roam" || p.roamShape === "circle"
      },
      roamShape: {
        type: ControlType.Enum,
        title: "Roam Shape",
        options: ["rectangle", "circle", "oval"],
        optionTitles: ["Rectangle", "Circle", "Oval"],
      ...
      hideType: {
        ...
        options: ["in-place", "scatter"],
        optionTitles: ["In Place", "Scatter"],
        hidden: (p) => p.hoverType !== "hide"
      }
    }
  },
  repulsionEnabled: {
  ...
  repulsionConfig: {
    type: ControlType.Object,
    title: "Repulsion",
    hidden: (props) => !props.repulsionEnabled,
    controls: {
      repulsionMode: {
        type: ControlType.Enum,
        title: "Mode",
        options: ["outside", "random"],
        optionTitles: ["Outside", "Random"],
